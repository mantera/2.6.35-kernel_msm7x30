...
msmsdcc_runtime_suspend(struct device *dev)
{
	...
	if (mmc) {
        ...
        // FihtdcCode@20111102 WeiChu add for SF4HC.B-1209 begin
#ifdef CONFIG_BROADCOM_BCM4329_WLAN
        //Broadcom patch , CSP [CASE#463653], chihchia 11.10.14 +++
        //rc = mmc_suspend_host(mmc);
        if(!mmc->card || (mmc->card && (mmc->card->type != MMC_TYPE_SDIO)))
        {
        printk("###### Hugh enter[%s] mmc_suspend_host\n",__func__);
            rc = mmc_suspend_host(mmc);
        }
        else
        {
            ...
            {
                printk("###### Hugh enter [%s] bef clk_disable(host->clk)\n",__func__);
                clk_disable(host->clk);
                printk("###### Hugh enter [%s] bef clk_disable(host->pclk)\n",__func__);
                clk_disable(host->pclk);
                host->clks_on = 0;
            }
        }
