...
static int bq275x0_battery_irqsetup(struct bq275x0_device_info *di)
{
    ...
    rc = request_any_context_irq(PM8058_GPIO_IRQ(PMIC8058_IRQ_BASE, di->pmic_BATGD), &bq275x0_battery_irqhandler,
    ...
    rc = request_any_context_irq(PM8058_GPIO_IRQ(PMIC8058_IRQ_BASE, di->pmic_BATLOW), &bq275x0_battery_irqhandler,
                 (IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING), 
                 "bq275x0-BATLOW", di);
    if (rc < 0) {
        dev_err(&di->client->dev,
