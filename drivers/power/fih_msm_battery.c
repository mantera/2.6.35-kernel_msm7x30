...
#define BATTERY_FULL_CHECK		4000 //chandler
...
static void msm_batt_update_psy_status(void)
{
	...
	/* 
	   FIH,chandler,11.9.22 : 
	   roll back for modem update bettery_level when they detect battery full. 
	*/
	msm_batt_info.battery_level 	= battery_level;
	DBG_LIMIT("update msm_batt_info.battery_level:(%d)\n",battery_level);
...
int msm_batt_get_batt_level(void)
{
    if (msm_batt_info.battery_level == BATTERY_LEVEL_FULL) {
        DBG_LIMIT("msm_batt_get_batt_level: Set 100 (modem say BATTERY_LEVEL_FULL)\n");
        return 100;
    } else {
        DBG_LIMIT("msm_batt_get_batt_level: msm_batt_info.battery_level(%d)\n",msm_batt_info.battery_level);
        return 99;
    }
}
...
static void msm_batt_update_batt_status(void)
{
    ...
    if (msm_batt_info.current_chg_source & (AC_CHG | USB_CHG)) {
        ...
        } else if (msm_batt_info.full || msm_batt_info.battery_level == BATTERY_LEVEL_FULL) {
            discharging_counter = 0;
...
void msm_batt_early_suspend(struct early_suspend *h)
{
	...
	if (msm_batt_info.batt_handle != INVALID_BATT_HANDLE) {
		rc = msm_batt_modify_client(msm_batt_info.batt_handle,
				BATTERY_FULL_CHECK, BATTERY_VOLTAGE_UP, //chandler
